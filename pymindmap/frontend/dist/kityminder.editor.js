...
angular.module('kityminderEditor').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/directive/layout/layout.html',
    "<div class=\"readjust-layout\"><a ng-click=\"minder.queryCommandState('resetlayout') === -1 || minder.execCommand('resetlayout') || close()\" class=\"btn-wrap\" ng-disabled=\"minder.queryCommandState('resetlayout') === -1\"><span class=\"btn-icon reset-layout-icon\"></span> <span class=\"btn-label\">{{ 'resetlayout' | lang: 'ui/command' }}</span></a></div>"
  );


  $templateCache.put('ui/directive/layoutrunall/layoutrunall.html',
    "<div class=\"km-btn-group arrange-group\"><div class=\"km-btn-item arrange-up\" ng-click=\"runallcode()\" title=\"{{ 'runallcode' | lang:'ui/command' }}\"><i class=\"km-btn-icon\"></i> <span class=\"km-btn-caption\">{{ 'runallcode' | lang: 'ui/command' }}</span></div><div class=\"km-btn-item arrange-down\" ng-click=\"killallcode()\" title=\"{{ 'killallcode' | lang:'ui/command' }}\"><i class=\"km-btn-icon\"></i> <span class=\"km-btn-caption\">{{ 'killallcode' | lang: 'ui/command' }}</span></div></div>"
  );
  ...
  $templateCache.put('ui/directive/navigator/navigator.html',
    ...
    "             }\"></div></div><div class=\"nav-btn zoom-out\" ng-click=\"minder.execCommand('zoomOut')\" title=\"{{ 'zoom-out' | lang : 'ui' }}\" ng-class=\"{ 'active' : getZoomRadio(zoom) == 1 }\"><div class=\"icon\"></div></div><div class=\"nav-btn hand\" ng-click=\"minder.execCommand('hand')\" title=\"{{ 'hand' | lang : 'ui' }}\" ng-class=\"{ 'active' : minder.queryCommandState('hand') == 1 }\"><div class=\"icon\"></div></div><div class=\"nav-btn glyphicon glyphicon-folder-close\" ng-click=\"$root.toggleNav=!$root.toggleNav\" title=\"{{ 'camera' | lang : 'ui' }}\"></div><div class=\"nav-btn nav-trigger\" ng-class=\"{'active' : isNavOpen}\" ng-click=\"toggleNavOpen()\" title=\"{{ 'navigator' | lang : 'ui' }}\"><div class=\"icon\"></div></div><div class=\"nav-btn camera\" title=\"{{ 'edit' | lang : 'ui' }}\" ng-click=\"view()\"><div class=\"icon\"></div></div></div><div class=\"nav-previewer\" ng-show=\"isNavOpen\"></div>"
  );


  $templateCache.put('ui/directive/noteBtn/noteBtn.html',
    "<div class=\"btn-group-vertical note-btn-group\" dropdown is-open=\"isopen\"><button type=\"button\" class=\"btn btn-default note-btn\" title=\"{{ 'note' | lang:'ui' }}\" ng-class=\"{'active': isopen}\" ng-click=\"addNote()\" ng-disabled=\"minder.queryCommandState('note') === -1\"></button> <button type=\"button\" class=\"btn btn-default note-btn-caption dropdown-toggle\" ng-disabled=\"minder.queryCommandState('note') === -1\" title=\"{{ 'note' | lang:'ui' }}\" dropdown-toggle><span class=\"caption\">{{ 'note' | lang:'ui' }}</span> <span class=\"caret\"></span> <span class=\"sr-only\">{{ 'note' | lang:'ui' }}</span></button><ul class=\"dropdown-menu\" role=\"menu\"><li><a href ng-click=\"addNote()\">{{ 'insertnote' | lang:'ui' }}</a></li><li><a href ng-click=\"minder.execCommand('note', null)\">{{ 'removenote' | lang:'ui' }}</a></li><li><a href ng-click=\"runNote()\">{{ 'runNote' | lang:'ui' }}</a></li><li><a href ng-click=\"killNote()\">{{ 'killNote' | lang:'ui' }}</a></li></ul></div>"
  );


  $templateCache.put('ui/directive/noteEditor/noteEditor.html',
    "<div class=\"panel panel-default\" ng-class=\"{'active': viewmode}\" ng-init=\"noteEditorOpen = false\" ng-show=\"noteEditorOpen\"><div class=\"panel-heading\"><h3 class=\"panel-title\">{{ 'note' | lang:'ui' }}</h3><i class=\"expand-note-editor glyphicon glyphicon-th-large\" ng-click=\"viewmode=!viewmode\"></i> <i class=\"close-note-editor glyphicon glyphicon-remove\" ng-click=\"closeNoteEditor()\"></i></div><div class=\"panel-body\"><div ng-show=\"noteEnabled\" ui-codemirror=\"{ onLoad: codemirrorLoaded }\" ng-model=\"noteContent\" ui-codemirror-opts=\"{\n" +
    "                breaks: true,\n" +
    "                lineWrapping : true,\n" +
    "                matchBrackets: true,\n" +
    "                mode: 'sql',\n" +
  ...
  $templateCache.put('ui/directive/sidebar/sidebar.html',
    "<div style=\"height:49px\"><div class=\"pull-left\" style=\"left: 10px;line-height: 32px;font-weight: bolder\">{{ 'minder' | lang: 'ui/sidebar' }}</div><div class=\"pull-right fa-chevron-right\" ng-click=\"refresh()\"><span class=\"glyphicon glyphicon-refresh\"></span></div><div class=\"pull-right btn-group-vertical\" dropdown is-open=\"isopen\"><button type=\"button\" class=\"btn btn-default image-btn-caption dropdown-toggle\" title=\"{{ 'operate' | lang: 'ui/sidebar' }}\" dropdown-toggle><span class=\"caption\">{{ 'operate' | lang: 'ui/sidebar' }}</span> <span class=\"caret\"></span> <span class=\"sr-only\">{{ 'operate' | lang: 'ui/sidebar' }}</span></button><ul class=\"dropdown-menu\" role=\"menu\"><li><a href ng-click=\"addMinder('addfolder')\">{{ 'addfolder' | lang: 'ui/sidebar' }}</a></li><li><a href ng-click=\"addMinder('addminder')\">{{ 'addminder' | lang: 'ui/sidebar' }}</a></li><li><a href ng-click=\"expandAll()\">{{ 'expandAll' | lang: 'ui/sidebar' }}</a></li><li><a href ng-click=\"collapseAll()\">{{ 'collapseAll' | lang: 'ui/sidebar' }}</a></li></ul></div></div><div ui-tree id=\"tree-root\"><ol ui-tree-nodes ng-model=\"nodes\"><li role=\"presentation\" ng-repeat=\"node in nodes\" ui-tree-node data-nodrag data-drag-enabled=\"false\" ng-include=\"'ui/directive/sidebar/sidebaritem.html'\"></li></ol></div>"
  );


  $templateCache.put('ui/directive/sidebar/sidebaritem.html',
  ...
  $templateCache.put('ui/dialog/folder/folder.tpl.html',
    "<div class=\"modal-header\"><h3 class=\"modal-title\">{{title}}</h3></div><div class=\"modal-body\"><form><div class=\"form-group\" id=\"link-url-wrap\"><label for=\"link-name\">名字：</label><input type=\"text\" class=\"form-control\" ng-model=\"sub\" ng-focus=\"this.value = sub\" id=\"link-name\" ng-disabled=\"editsub\" placeholder=\"必填：天天都有好心情\"></div><div class=\"form-group\" id=\"link-url-wrap\"><label for=\"link-name\">地址：</label></div><div class=\"select form-group\" id=\"link-url-wrap\"><select ng-if=\"!editpath\" ng-model=\"selectedplace.selected\" ng-options=\"x for x in places\"></select><input ng-if=\"editpath\" type=\"text\" class=\"form-control\" ng-model=\"path\" ng-disabled=\"editpath\"></div></form></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" ng-click=\"ok()\">确定</button> <button class=\"btn btn-warning\" ng-click=\"cancel()\">取消</button></div>"
  );
...
angular.module('kityminderEditor')
	.provider('config',  function() {
		...
		this.config = {
			...
			// 放大缩小比例
			zoom: [10, 20, 30, 50, 80, 100, 120, 150, 200, 300,500 ],
			// 图片上传接口,获得跨域图片接口
			picture: 'api/picture',
			// 获得minder内容接口
			getminder:'api/minder',
			// websocket端口
			socketminder:'api/socketminder',
			// 执行code代码
			runcode:'api/exec',
			// folder端口
			folder:'api/folder'
    ...
    .directive('navigator', ['memory', 'config', function(memory, config) {
        return {
            ...
            link: function(scope) {
                ...
                scope.view = function(){
                    scope.$root.viewmode = !scope.$root.viewmode;
                    // console.log(scope.$root.toggleNav)
                }
    ...
    .directive('resourceEditor', function () {
        return {
            ...
            controller: ["$scope", function ($scope) {
                ...
                minder.on('interactchange', function () {
                    // console.log(minder.getUsedResource())
                    var enabled = $scope.enabled = minder.queryCommandState('resource') != -1;
                    var selected = enabled ? minder.queryCommandValue('resource') : [];
